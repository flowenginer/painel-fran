...
//
// Gerência de usuários do CRM da clínica pelo admin (dona). Só pode ser
// chamada por um usuário cujo perfil em `usuarios` tenha role = 'admin' e
// ativo = true.
//
// Ações (no corpo JSON, campo "action"):
//   - listar                          → lista todos os perfis (com a unidade)
//   - criar    { email, password, nome?, role?, unidade_id?, permissoes? }
//   - atualizar{ id, nome?, role?, ativo?, unidade_id?, permissoes? }
...
const SELECT_PERFIL =
  "id,nome,email,role,ativo,unidade_id,permissoes,created_at,updated_at";
...
async function lerPerfil(env: SupabaseEnv, id: string): Promise<PerfilRow | null> {
  const resp = await rest(
    env,
    "GET",
    `/usuarios?id=eq.${id}&select=${SELECT_PERFIL}`
  );
...
function validarRole(role: unknown): "admin" | "atendente" {
  return role === "admin" ? "admin" : "atendente";
}

// Converte unidade_id vindo do corpo em number | null (admin = null = todas).
function normalizarUnidade(v: unknown): number | null {
...
// Conta admins ativos (para as salvaguardas do "último admin").
async function contarAdminsAtivos(env: SupabaseEnv): Promise<number> {
  const resp = await rest(
    env,
    "GET",
    "/usuarios?role=eq.admin&ativo=eq.true&select=id",
    undefined,
    { Prefer: "count=exact" }
  );
  ...
  try {
    ...
    if (action === "listar") {
      const resp = await rest(
        env,
        "GET",
        `/usuarios?select=${SELECT_PERFIL}&order=created_at.asc`
      );
      if (!resp.ok) {
    ...
    if (action === "criar") {
      ...
      // Admin enxerga todas as unidades → unidade_id fica NULL.
      const unidadeId = role === "admin" ? null : normalizarUnidade(body.unidade_id);
      ...
      if (role === "atendente" && unidadeId === null) {
        return jsonResponse(
          { error: "Selecione a unidade da atendente" },
          400
        );
      }
    ...
    if (action === "atualizar") {
      ...
      if (body.unidade_id !== undefined)
        patch.unidade_id = normalizarUnidade(body.unidade_id);
      if (body.permissoes !== undefined)
        patch.permissoes = normalizarPermissoes(body.permissoes);

      // Se o papel final for admin, força unidade_id = NULL (enxerga todas).
      const roleFinal = patch.role !== undefined ? patch.role : alvo.role;
      if (roleFinal === "admin") patch.unidade_id = null;
      ...
      if (removendoAdmin && alvo.role === "admin" && alvo.ativo) {
        if ((await contarAdminsAtivos(env)) <= 1) {
          return jsonResponse(
            {
              error:
    ...
    if (action === "remover") {
      ...
      if (!id) return jsonResponse({ error: "id é obrigatório" }, 400);
      if (id === callerId) {
        return jsonResponse({ error: "Você não pode remover a si mesmo" }, 409);
      }
      const alvo = await lerPerfil(env, id);
