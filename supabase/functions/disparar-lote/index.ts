...
// `conectadas` é passado, o rodízio só considera essas instâncias. Retorna
// null se nenhum canal elegível — nesse caso o n8n usa o número padrão dele.
...
// Checa, via webhook de status do n8n (UAZAPI), se uma instância está
// conectada no WhatsApp. Timeout curto para não travar o disparo.
...
// Lista as instâncias de disparo que estão de fato CONECTADAS agora. Atualiza
// o cache fran_canais.conectado. Retorna null quando a checagem não pôde
// rodar (webhook não configurado / sem candidatos) — aí o rodízio não filtra.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // SÓ entre os números conectados no WhatsApp agora, e injeta instancia+token
    // no payload. O n8n envia pela instância e grava o canal na fran_memory
    // (para a conversa "grudar" naquele número).
